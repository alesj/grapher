 ...
 */
public abstract class SingleGraphCreator<T> extends AbstractGraphCreator<T> {
    private T singleton;

    public void initialValue(final String value) {
        singleton = toName(value);
    }
    ...
    protected void createCells(HttpServletRequest request, DependencyType<T> dtype, Map<Object, GraphCell> cells, Set<Object> objects, Map attributes, ConnectionSet cs, DependencyFilter<T> filter) {
        DependencyTarget<T> context = getDependencyTarget(request, singleton);
        if (context == null)
            throw new IllegalArgumentException("No such context: " + singleton);
    ...
    public String toString() {
        return super.toString() + "[singleton=" + singleton + "]";
    }
}